...
class DetailsWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () async {
        Navigator.pushNamed(context, RouteList.details,arguments: [{'button':button}]).then((value){
          ...
          children: [
           Column(
             ...
             children: [
               ...
               context.watch<ProviderReminder>().buttonDetails?Row(
                 children: [
                   context.watch<ProviderReminder>().timeDetails?Text('${DateFormat('HH:mm,').format(DateTime.fromMillisecondsSinceEpoch(context.watch<ProviderReminder>().valuesTime))}',style: TextStyle(color: Colors.black87.withOpacity(0.6),fontSize: ScreenUtil().setSp(14))):Text(''),
                   Text('${DateFormat('dd/MM/yyyy').format(DateTime.fromMillisecondsSinceEpoch(context.watch<ProviderReminder>().valuesTime))}',style: TextStyle(color: Colors.black87.withOpacity(0.6),fontSize: ScreenUtil().setSp(14)))
